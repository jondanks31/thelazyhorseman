/**
 * The masthead lockup. "lazy" is drawn as paths rather than set in the
 * font, so it never reflows or flashes while a webfont loads.
 *
 * Server component: no state, no handlers, nothing to hydrate.
 */
export default function Wordmark({ tag }: { tag?: string }) {
  return (
    <header className="gate-head">
      <a className="logo" href="https://www.thelazyhorseman.com/">
        The
        <svg className="mark" aria-hidden="true" focusable="false" viewBox="20 18 262 145">
      ...
      {tag && <span className="gate-tag">{tag}</span>}
    </header>
  );
}
